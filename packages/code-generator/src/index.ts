...
export default class CodeGenerator {
    ...
    _handleEvents(events: any, isRecordingVideo = false, isLiveProgress = false){
        ...
        if(events[0] && events[0].event_type!==ACTIONS_IN_TEST.SET_DEVICE) {
            ...
            if(isLiveProgress) {
                code += `await logStep('${ACTIONS_IN_TEST.SET_DEVICE}', {status: 'DONE', message: '${ACTION_DESCRIPTIONS[ACTIONS_IN_TEST.SET_DEVICE]({value: device.name})}'}, {name: '${device.name}', width: ${width}, height: ${height}, userAgent: '${userAgent.value}'});\n`;
            }
        ...
        for (let i = 0; i < events.length; i++) {
            ...
            switch (event_type) {
                case ACTIONS_IN_TEST.NAVIGATE_URL:
                    ...
                    if(isLiveProgress){
                        code += `await logStep('${ACTIONS_IN_TEST.NAVIGATE_URL}', {status: 'DONE', message: '${ACTION_DESCRIPTIONS[ACTIONS_IN_TEST.NAVIGATE_URL]({value: value})}'});\n`;
                    }
                    if(isRecordingVideo){
                ...
                case ACTIONS_IN_TEST.CLICK:
                    ...
                    if(isLiveProgress){
                        code += `await logStep('${ACTIONS_IN_TEST.CLICK}', {status: 'DONE', message: '${ACTION_DESCRIPTIONS[ACTIONS_IN_TEST.CLICK]({selector: selectors[0].value})}'}, {selector: '${selectors[0].value}'});\n`;
                    }
                    if(isRecordingVideo){
                        code+= `await sleep(DEFAULT_SLEEP_TIME);\n`;
                    }
                ...
                case ACTIONS_IN_TEST.HOVER:
                    ...
                    if(isLiveProgress){
                        code += `await logStep('${ACTIONS_IN_TEST.HOVER}', {status: 'DONE', message: '${ACTION_DESCRIPTIONS[ACTIONS_IN_TEST.HOVER]({selector: selectors[0].value})}'}, {selector: '${selectors[0].value}'});\n`;
                    }
                    if(isRecordingVideo){
                        code+= `await sleep(DEFAULT_SLEEP_TIME);\n`;
                    }
                ...
                case ACTIONS_IN_TEST.ELEMENT_SCREENSHOT:
                    ...
                    if(isLiveProgress){
                        code += `await logStep('${ACTIONS_IN_TEST.ELEMENT_SCREENSHOT}', {status: 'DONE', message: '${ACTION_DESCRIPTIONS[ACTIONS_IN_TEST.ELEMENT_SCREENSHOT]({selector: selectors[0].value})}'}, {selector: '${selectors[0].value}'});\n`;
                    }
                    if(isRecordingVideo){
                        code+= `await sleep(DEFAULT_SLEEP_TIME);\n`;
                    }
                ...
                case ACTIONS_IN_TEST.PAGE_SCREENSHOT:
                    ...
                    if(isLiveProgress){
                        code += `await logStep('${ACTIONS_IN_TEST.PAGE_SCREENSHOT}', {status: 'DONE', message: '${ACTION_DESCRIPTIONS[ACTIONS_IN_TEST.PAGE_SCREENSHOT]()}'}, {selector: 'body'});\n`;
                    }
                    if(isRecordingVideo){
                        code+= `await sleep(DEFAULT_SLEEP_TIME);\n`;
                    }
                ...
                case ACTIONS_IN_TEST.SCROLL_TO_VIEW:
                    code += `await page.waitForSelector('${selectors[0].value}', {state: "attached"});\nconst stv_${i} = await page.$('${selectors[0].value}');\nawait stv_${i}.scrollIntoViewIfNeeded();\n`
                    if(isLiveProgress){
                        code += `await logStep('${ACTIONS_IN_TEST.SCROLL_TO_VIEW}', {status: 'DONE', message: '${ACTION_DESCRIPTIONS[ACTIONS_IN_TEST.SCROLL_TO_VIEW]({selector: selectors[0].value})}'}, {selector: '${selectors[0].value}'});\n`;
                    }
                    if(isRecordingVideo){
                        code+= `await sleep(DEFAULT_SLEEP_TIME);\n`;
                    }
                ...
                case ACTIONS_IN_TEST.INPUT:
                    ...
                    if(isLiveProgress){
                        code += `await logStep('${ACTIONS_IN_TEST.INPUT}', {status: 'DONE', message: '${ACTION_DESCRIPTIONS[ACTIONS_IN_TEST.INPUT]({selector: selectors[0].value, value})}'}, {selector: '${selectors[0].value}', value: '${value}'});\n`;
                    }
                    if(isRecordingVideo){
                        code+= `await sleep(DEFAULT_SLEEP_TIME);\n`;
                    }
                ...
                case ACTIONS_IN_TEST.EXTRACT_INFO:
                    ...
                    if(isLiveProgress){
                        code += `await logStep('${ACTIONS_IN_TEST.EXTRACT_INFO}', {status: 'DONE', message: '${ACTION_DESCRIPTIONS[ACTIONS_IN_TEST.EXTRACT_INFO]({selector: selectors[0].value})}'}, {selector: '${selectors[0].value}'});\n`;
                    }
                    if(isRecordingVideo){
                        code+= `await sleep(DEFAULT_SLEEP_TIME);\n`;
                    }
                ...
                case ACTIONS_IN_TEST.ASSERT_ELEMENT:
                    ...
                    code += ` `;
                    if(isLiveProgress){
                        code += `await logStep('${ACTIONS_IN_TEST.ASSERT_ELEMENT}', {status: 'DONE', message: '${ACTION_DESCRIPTIONS[ACTIONS_IN_TEST.ASSERT_ELEMENT]({selector: selectors[0].value})}'}, {selector: '${selectors[0].value}'});\n`;
                    }
                    if(isRecordingVideo){
                        code+= `await sleep(DEFAULT_SLEEP_TIME);\n`;
                    }
