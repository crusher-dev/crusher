...
JavaScriptCompiler.prototype = {
  ...
  depthedLookup: function depthedLookup(name) {
    return [this.aliasable('containers.lookup'), '(depths, "', name, '")'];
  },
  ...
  compile: function compile(environment, options, context, asObject) {
    ...
    if (!this.decorators.isEmpty()) {
      ...
      this.decorators.prepend('var decorators = containers.decorators;\n');
      this.decorators.push('return fn;');

      if (asObject) {
        this.decorators = Function.apply(this, ['fn', 'props', 'container', 'depth0', 'data', 'blockParams', 'depths', this.decorators.merge()]);
      } else {
        this.decorators.prepend('function(fn, props, containers, depth0, data, blockParams, depths) {\n');
  ...
  appendEscaped: function appendEscaped() {
    this.pushSource(this.appendToBuffer([this.aliasable('containers.escapeExpression'), '(', this.popStack(), ')']));
  },
  ...
  lookupData: function lookupData(depth, parts, strict) {
    if (!depth) {
      ...
      this.pushStackLiteral('containers.data(data, ' + depth + ')');
    }
  ...
  resolvePossibleLambda: function resolvePossibleLambda() {
    this.push([this.aliasable('containers.lambda'), '(', this.popStack(), ', ', this.contextName(0), ')']);
  },
  ...
  invokePartial: function invokePartial(isDynamic, name, indent) {
    ...
    options.decorators = 'containers.decorators';
    ...
    this.push(this.source.functionCall('containers.invokePartial', '', params));
  },
  ...
  programExpression: function programExpression(guid) {
    ...
    return 'containers.program(' + programParams.join(', ') + ')';
  },
  ...
  setupHelper: function setupHelper(paramSize, name, blockHelper) {
    ...
    var foundHelper = this.nameLookup('helpers', name, 'helper'),
        callContext = this.aliasable(this.contextName(0) + ' != null ? ' + this.contextName(0) + ' : (containers.nullContext || {})');
  ...
  setupParams: function setupParams(helper, paramSize, params) {
    ...
    if (program || inverse) {
      options.fn = program || 'containers.noop';
      options.inverse = inverse || 'containers.noop';
    }
...
function strictLookup(requireTerminal, compiler, parts, type) {
  ...
  if (requireTerminal) {
    return [compiler.aliasable('containers.strict'), '(', stack, ', ', compiler.quotedString(parts[i]), ')'];
  } else {
    return stack;
  }
}
