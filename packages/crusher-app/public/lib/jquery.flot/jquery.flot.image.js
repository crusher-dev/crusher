...
A more low-level helper, $.plot.image.resources(urls, callback) is also included.
...
(function ($) {
	...
	$.plot.image.load = function (urls, callback) {
		...
		$.each(urls, function (i, url) {
			...
			$('<img loading="lazy"/>').load(handler).error(handler).attr('src', url);
		});
	};
	...
	function drawSeries(plot, ctx, series) {
		...
		for (var i = 0; i < points.length; i += ps) {
			...
			// IE6 (false even after resources event)
