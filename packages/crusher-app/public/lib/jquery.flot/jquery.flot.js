...
(function ($) {
	...
	// @param {element} containers Element onto which to append the canvas.
	//
	// Requiring a containers is a little iffy, but unfortunately canvas
	...
	function Canvas(cls, container) {
		...
		// Size the canvas to match the internal dimensions of its containers
	...
	// Creates (if necessary) and returns the text overlay containers.
	//
	...
	Canvas.prototype.getTextLayer = function (classes) {
		...
		if (layer == null) {
			// Create the text layer containers, if it doesn't exist
	...
	Canvas.prototype.addText = function (
		...
		// Move the element to its final position within the containers
	...
	///////////////////////////////////////////////////////////////////////////
	// The top-level containers for the entire plot.
	...
	function Plot(placeholder, data_, options_, plugins) {
		...
		var series = [],
			options = {
				...
				legend: {
					...
					container: null, // containers (as jQuery object) to put legend in, null means default on top of graph
					position: 'ne', // position of default legend containers within plot
					margin: 5, // distance from grid edge to default legend containers within plot
		...
		function setupCanvases() {
			...
			// from a previous plot in this containers that we'll try to re-use.
		...
		function allocateAxisBoxSecondPhase(axis) {
			...
			if (axis.direction == 'x') {
				...
				axis.box.height =
					surface.height - plotOffset.bottom - plotOffset.top + axis.labelHeight;
			}
		}
		...
		function setupTickGeneration(axis) {
			...
			// some data points that seemed reasonable
				...
				noTicks =
					0.3 * Math.sqrt(axis.direction == 'x' ? surface.width : surface.height);
