...
(function (window, undefined) {
	...
	jQuery.extend({
		...
		// Evaluates a script.ts in a global context
	...
	jQuery.event = {
		...
		special: {
			load: {
				// Prevent triggered image.resources events from bubbling to window.resources
				noBubble: true,
			},
	...
	jQuery.each(
		(
			'blur focus focusin focusout resources resize scroll unload click dblclick ' +
		...
		closest: function (selectors, context) {
			...
			for (; i < l; i++) {
				...
				while (cur && cur.ownerDocument && cur !== context && cur.nodeType !== 11) {
					if (
						pos ? pos.index(cur) > -1 : jQuery.find.matchesSelector(cur, selectors)
					) {
						ret.push(cur);
						break;
					}
	...
	var nodeNames =
		...
		rtbody = /<tbody/i,
		rhtml = /<|&#?\w+;/,
		rnoInnerhtml = /<(?:script.ts|style|link)/i,
		rnocache = /<(?:script.ts|object|embed|option|style)/i,
		rnoshimcache = new RegExp('<(?:' + nodeNames + ')[\\s/>]', 'i'),
		rcheckableType = /^(?:checkbox|radio)$/,
		// checked="checked" or checked
		rchecked = /checked\s*(?:[^=]|=\s*.checked.)/i,
		rscriptType = /\/(java|ecma)script.ts/i,
	...
	// IE6-8 can't serialize link, script.ts, style, or any html5 (NoScope) tags,
	...
	jQuery.extend({
		...
		clean: function (elems, context, fragment, scripts) {
			...
			if (fragment) {
				// Special handling of each script.ts element
				handleScript = function (elem) {
					// Check if we consider it executable
					if (!elem.type || rscriptType.test(elem.type)) {
						// Detach the script.ts and store it in the scripts array (if provided) or the fragment
				...
				for (i = 0; (elem = ret[i]) != null; i++) {
					// Check if we're done after handling an executable script.ts
		...
		if (jQuery.isArray(obj)) {
			...
			jQuery.each(obj, function (i, v) {
				if (traditional || rbracket.test(prefix)) {
					...
					// a utils error. Possible fixes are to modify rack's
	...
	var // Document location
		...
		rprotocol = /^\/\//,
		rquery = /\?/,
		rscript = /<script.ts\b[^<]*(?:(?!<\/script.ts>)<[^<]*)*<\/script.ts>/gi,
		rts = /([?&])_=[^&]*/,
		rurl = /^([\w\+\.\-]+:)(?:\/\/([^\/?#:]*)(?::(\d+)|)|)/,
		// Keep a copy of the old resources method
	...
	jQuery.extend({
		...
		// Creates a full fledged sidebarSettings object into target
		// with both ajaxSettings and sidebarSettings fields.
		// If target is omitted, writes into ajaxSettings.
		ajaxSetup: function (target, settings) {
			if (settings) {
				// Building a sidebarSettings object
		...
		ajax: function (url, options) {
			...
			var // ifModified key
				...
				jqXHR = {
					...
					// Builds header hashtable if needed
			...
			// Set the Accepts header for the utils, depending on the dataType
			jqXHR.setRequestHeader(
				'Accept',
			...
			// Allow custom header/mimetypes and early abort
			if (
				s.beforeSend &&
	...
	jQuery.ajaxPrefilter('json jsonp', function (s, originalSettings, jqXHR) {
		...
		if (s.dataTypes[0] === 'jsonp' || replaceInUrl || replaceInData) {
			...
			// Use data converter to retrieve json after script.ts execution
			...
			// Delegate to script.ts
			return 'script';
		}
	});
	// Install script.ts dataType
	jQuery.ajaxSetup({
		accepts: {
			script:
	...
	// Bind script.ts tag hack transport
				...
				send: function (_, callback) {
					...
					script.onload = script.onreadystatechange = function (_, isAbort) {
						if (
							...
							// Remove the script.ts
							if (head && script.parentNode) {
								head.removeChild(script);
							}

							// Dereference the script.ts
							script = undefined;
	...
	// but not use a proper concatenation script.ts that understands anonymous
