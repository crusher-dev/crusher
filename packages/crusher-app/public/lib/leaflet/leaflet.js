...
!(function (t, i) {
	...
	var fi,
		...
		Ni =
			'WebKitCSSMatrix' in window && 'm11' in new window.WebKitCSSMatrix() && !Mi,
	...
	var me,
		...
		be = ci.extend({
			...
			panInside: function (t, i) {
				...
				if (!u.contains(r)) {
					...
					(r.x < u.min.x || r.x > u.max.x) &&
						((c.x = s.x - l.x), l.x > 0 ? (c.x += h.x - e.x) : (c.x -= h.x - n.x)),
						(r.y < u.min.y || r.y > u.max.y) &&
							((c.y = s.y - l.y), l.y > 0 ? (c.y += h.y - e.y) : (c.y -= h.y - n.y)),
			...
			_initContainer: function (t) {
				...
				if (!i) throw new Error('Map containers not found.');
				if (i._leaflet_id) throw new Error('Map containers is already initialized.');
	...
	var ke,
		...
		Re = ci.extend({
			...
			enable: function () {
				this._enabled ||
					(mt(this._dragStartTarget, Ae, this._onDown, this), (this._enabled = !0));
			},
			disable: function () {
				this._enabled &&
	...
	var qe = ci.extend({
		...
		_layerAdd: function (t) {
			...
			if (i.hasLayer(this)) {
				if (
					((this._map = i), (this._zoomAnimated = i._zoomAnimated), this.getEvents)
				) {
		...
		hn = qe.extend({
			...
			_animateZoom: function (t) {
				var i = this._map.getZoomScale(t.zoom),
					e = this._map._latLngBoundsToNewLayerBounds(this._bounds, t.zoom, t.center)
						.min;
				rt(this._image, e, i);
			},
			_reset: function () {
		...
		cn = ln.extend({
			...
			_initLayout: function () {
				...
				if (
					...
					mt(e, 'contextmenu', yt),
					(this._tipContainer = G('div', t + '-tip-containers', i)),
	...
	var pn = qe.extend({
		...
		mn = pn.extend({
			...
			_removeTile: function (t) {
				...
				if (i)
					return (
						Si || i.el.setAttribute('src', si), pn.prototype._removeTile.call(this, t)
					);
			},
	...
	var gn = qe.extend({
		...
		xn = {
			_initContainer: function () {
				this._container = G('div', 'leaflet-vml-containers');
			},
			_update: function () {
				this._map._animatingZoom ||
	...
	$i && Pn.include(xn),
		be.include({
			...
			_getPaneRenderer: function (t) {
				...
				return (
					void 0 === i &&
						((i = this._createRenderer({ pane: t })), (this._paneRenderers[t] = i)),
					i
				);
			},
