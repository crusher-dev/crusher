		...
		return (
			...
			(d = {
							...
							(z.clone = function () {
								if (z.calWidth(!0) > o) {
									for (
										var b = 0, c = 0, d = 0;
										d < h.length &&
										((b += parseInt(g.find('.lslide').eq(d).width()) + e.slideMargin),
										c++,
										!(b >= o + e.slideMargin));
										d++
									);
									...
									for (var k = g.find('.clone.right').length; f > k; k++)
										g
											...
											q++;
									for (
										var l = g.find('.lslide').length - g.find('.clone.left').length;
					...
					if (
						((z.createPager = function () {
							...
							for (d = 0; c > d; d++) {
								'slide' === e.mode &&
									(e.autoWidth
										? (h += (parseInt(b.eq(d).width()) + e.slideMargin) * e.slideMove)
							...
							k.find('.lSPager').html(f),
								e.gallery === !0 &&
									...
									k
										.find('.lSPager')
										.css({ property: u + 'px', 'transition-duration': e.speed + 'ms' }),
									e.vertical === !0 &&
										p
											.parent()
											.css('padding-right', e.vThumbWidth + e.galleryMargin + 'px'),
							...
							l.first().addClass('active'),
								l.on('click', function () {
									return (
										e.loop === !0 && 'slide' === e.mode
											? (q += l.index(this) - k.find('.lSPager').find('li.active').index())
											: (q = l.index(this)),
										g.mode(!1),
				...
				active: function (a, b) {
					...
					if (q * e.slideMove < l) {
						...
						b === !0 && ((d = a.length), (f = d - 1), c + 1 >= d && (c = f)),
							e.loop === !0 &&
								'slide' === e.mode &&
								((c = b === !0 ? q - g.find('.clone.left').length : q * e.slideMove),
								b === !0 &&
									((d = a.length),
									(f = d - 1),
									c + 1 === d ? (c = f) : c + 1 > d && (c = 0))),
				...
				slideThumb: function () {
					...
					'slide' === e.mode &&
						e.loop === !0 &&
						(b >= c.children().length ? (b = 0) : 0 > b && (b = c.children().length));
					...
					if (y) {
						...
						p.on('touchstart', function (a) {
							...
							p.on('touchmove', function (d) {
								...
								e.vertical === !0
									? (3 * h > g && d.preventDefault(), a.touchMove(c.pageY, b.pageY))
									: (3 * g > h && d.preventDefault(), a.touchMove(c.pageX, b.pageX));
							}),
					...
					n === !1 &&
						('slide' === e.mode
							? d.doCss() &&
							  ...
							  '' !== e.cssEasing && p.css('transition-timing-function', e.cssEasing))
							: d.doCss() &&
