define(['eve'], function (eve) {
     ...
     * from this instance will be bound to this canvas.
     > Parameters
     **
     - containers (HTMLElement|string) DOM element or its ID which is going to be a parent for drawing surface
     - width (number)
     - height (number)
	...
	elproto.pause = function (anim) {
		for (var i = 0; i < animationElements.length; i++)
			if (
				...
				if (
					eve('raphael.anim.pause.' + this.id, this, animationElements[i].anim) !==
					false
				) {
	...
	elproto.stop = function (anim) {
		for (var i = 0; i < animationElements.length; i++)
			if (
				...
				if (
					eve('raphael.anim.stop.' + this.id, this, animationElements[i].anim) !==
					false
				) {
					animationElements.splice(i--, 1);
				}
