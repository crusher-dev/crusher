...
function DownloadButton() {
	...
	const DownloadButton = useCallback(({downloadLink,label, icon}) =>{
		return <a href={downloadLink} onClick={setDownload.bind(this, true)}><Button className={'mt-28'} css={css`width: 182rem;`}>
			<div className={'flex items-center justify-center'}>
				<AppleSVG className={'mr-12'} /><span className={'mt-2'}>{label}</span>
			</div>
		</Button>
		</a>;
	},[])
	...
	if(osType === OS.Linux){
		...
		return (<div className={"flex flex-col items-center"}>
			...
			{isDownloading && (
				<div className={"mt-32 text-13"}>Iniitiaing download, if it doesn't download. Open link in new tab.
				</div>
			)}
		</div>);
	}
