...
(function (mod) {
	...
	CodeMirror.defineMode("javascript", function (config, parserConfig) {
		...
		function statement(type, value) {
			...
			if (type === "keyword d") return cx.stream.match(/^\s*$/, false) ? cont() : cont(pushlex("stat"));
			...
			if (type === "class" || (isTS && value === "interface")) {
				cx.marked = "keyword";
				return cont(pushlex("form", type === "class" ? type : value));
			}
			if (type === "variable") {
		...
		function expressionInner(type, value, noComma) {
			...
			if (type === "quasi") return pass(quasi);
		...
		function maybeoperatorNoComma(type, value, noComma) {
			var me = !noComma ? maybeoperatorComma : maybeoperatorNoComma;
			var expr = !noComma ? expression : expressionNoComma;
			if (type === "=>") return cont(pushcontext);
			if (type === "operator") {
				if (/\+\+|--/.test(value) || (isTS && value === "!")) return cont(me);
				if (isTS && value === "<" && cx.stream.match(/^([^<>]|<[^<>]*>)*>\s*\(/, false)) return cont(pushlex(">"));
				if (value === "?") return cont(expression);
				return cont(expr);
			}
			if (type === "quasi") {
		...
		function maybeelse(type, value) {
			if (type === "keyword b" && value === "else") return cont(pushlex("form", "else"));
		}
		...
		function classBody(type, value) {
			...
			if (type === "number" || type === "string") return cont(classfield);
			if (type === "[") return cont(expression);
			if (value === "*") {
				cx.marked = "keyword";
				return cont(classBody);
