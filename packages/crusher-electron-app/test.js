...
(async () => {
	...
	const webViewConfig = await axios.get("http://127.0.0.1:9112/json").then((res) => {
		return res.data.find((item) => {
			return item.type === "webview";
		});
	});
	// if (!webViewConfig) throw new Error("Nothing to connect to");

	const browser = await playwright.chromium.connectOverCDP("http://localhost:9112/", { customBrowserName: "electron-webview" });
	...
	const webViewPage = await pagesMap.findIndex((url) => {
	...
	const sitesList = ["https://google.com", "https://youtube.com", "https://twitter.com", "https://playwright.dev", "https://stackoverflow.com"];
	for (site of sitesList) {
		console.log("Current url", await page.url());
		console.log("Starting navigation to", site);
		await page.goto(site);
		console.log("Finished navigation to", await page.url());
	}
	// await crusherRunnerActionManager.runActions(getMainActions(actions), browser, page);
	// const elementHandle = await page.$("body");
	// console.log(await elementHandle.innerHTML());
})();
