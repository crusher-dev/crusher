...
export function AssertModalContent({
	handleCloseCallback,
	seoMeta,
	attributes,
}: any) {
...
function Row({ name, state, setState, attributes, isValid }: any) {
	...
	const onNameChange = (event: any) => {
		const currentNameIndex = attributes.find((attribute: any) => {
			return (
				attribute.name === event.target.options[event.target.selectedIndex].value
			);
		});
		...
		setState({
			...
			attributes: {
				...state.attributes,
				[name.toString().toLowerCase()]: event.target.options[
	...
	const attributesOut = attributes
		? attributes.map((attr: any, index: number) => {
				return (
					<option key={index} label={attr.name} value={attr.name}>
						{attr.name}
					</option>
				);
		  })
			...
			<div style={{ flex: 1, display: "flex", alignItems: "center" }}>
				...
				{isValid ? (
					<img
						...
						style={{ width: "1.4rem", marginLeft: "1.1rem" }}
					/>
				) : (
					<img
						src={chrome.runtime.getURL("icons/cross.svg")}
						style={{ width: "0.9rem", marginLeft: "1.1rem" }}
					/>
				)}
			</div>
			<select
...
function MiddleSection({ handleCloseCallback, seoMeta, attributes }: any) {
	...
	const rowsOut = state.rows.map((row: any) => {
		...
		return (
			<Row
				isValid={state.validMap[id]}
				name={id}
				attributes={attributes}
				key={id}
				state={state}
				setState={setStateMiddleware}
			/>
		);
	});
