  ...
  return (
    <div className={"middleRow"} style={styles.middleRow}>
      ...
      <input
        ...
        value={state.assertValues[name.toLowerCase()]}
        style={{ ...styles.input }}
        placeholder={`Your assertion ${name}`}
      />
    </div>
  );
}
...
function MiddleSection({ handleCloseCallback, seoMeta }: any) {
  ...
  useEffect(() => {
    setState({
      ...
      assertValues: {
        ...state.assertValues,
        title: state.assertValues.title
          ? state.assertValues.title
          : seoMeta.title,
        description: state.assertValues.description
          ? state.assertValues.description
          : seoMeta.description,
      },
    });
  }, [seoMeta]);
