...
export default class MonitoringService {
	...
	async saveSettingsForProject(settings: MonitoringSettings, projectId: number) {
		...
		if (!monitoringSettings) {
			await this.dbManager.insert(`INSERT INTO monitoring_settings SET ?`, settings);
		}
	...
	async addMonitoringForProject(settings: MonitoringSettings, projectId: number) {
		return this.dbManager.insert(`INSERT INTO monitoring_settings SET ?`, settings);
	}
	...
	async getQueuedMonitorings(): Promise<Array<iMonitoringSettings>> {
		return this.dbManager.fetchAllRows(`SELECT * FROM monitoring_settings WHERE UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(last_cron_run) > test_interval`);
	}
