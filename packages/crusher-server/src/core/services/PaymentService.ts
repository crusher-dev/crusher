...
export default class PaymentService {
    ...
    async getPlanData(planIds: number[]){
        return this.dbManager.fetchData(
            `SELECT * from pricing_plan where id IN (?)`,
            [planIds],
        );
    }
    ...
    async removePlan( pricingLogId){
        const {stripe_subscription_item_id:stripeSubscriptionItemId} = await this.dbManager.fetchSingleRow(`Select * from team_pricing_log where id=?`,pricingLogId)
    ...
    async startBilling(team_id, crusherPlanIds ): Promise<object> {
        ...
        const updateTeamPromise =  this.dbManager.fetchSingleRow(`UPDATE teams SET type='PAID', stripe_subscription_id=? WHERE id = ?`, [subscriptionData.id, team_id]);
