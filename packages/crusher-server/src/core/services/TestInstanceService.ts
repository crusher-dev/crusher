...
export default class TestInstanceService {
	...
	async createNewTestInstance(details: CreateTestInstanceRequest) {
		...
		return this.dbManager.insert(`INSERT INTO test_instances SET ?, created_at = NOW()`, {
			job_id: jobId,
			test_id: testId,
			code: code,
			platform: platform,
	...
	async getAllTestInstances(testId: number) {
		return this.dbManager.fetchAllRows("SELECT * FROM test_instances WHERE test_id = ? ORDER BY created_at DESC", [testId]);
	}
