...
export default class JobReportServiceV2 {
	...
	async getTotalJobsReportsCountInProject(projectId: number, trigger: JobTrigger){
		if(trigger === JobTrigger.MANUAL){
			return this.dbManager.fetchSingleRow(`SELECT count(*) as totalCount FROM job_reports, jobs WHERE job_reports.project_id = ? AND jobs.id = job_reports.job_id AND jobs.trigger = ?`, [projectId, JobTrigger.MANUAL]);
		} else if(trigger === JobTrigger.MONITORING){
			return this.dbManager.fetchSingleRow(`SELECT count(*) as totalCount FROM job_reports, jobs WHERE job_reports.project_id = ? AND jobs.id = job_reports.job_id AND (jobs.trigger = ? OR jobs.trigger = ?) `, [projectId, JobTrigger.CLI, JobTrigger.CRON]);
		} else {
			// All
