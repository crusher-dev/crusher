...
export class BuildsController {
	...
	public async getBuildsList(
		...
		@QueryParams() params: { triggerType?: BuildTriggerEnum; triggeredBy?: number; search?: string; page?: number; status?: BuildReportStatusEnum },
	): Promise<IProjectBuildListResponse & { availableAuthors: Pick<KeysToCamelCase<IUserTable>, "name" | "email" | "id">[] }> {
		...
		const buildsList = buildsData.list.map((buildData) => ({
			...
			commentCount: buildData.commentCount || 0,
		}));
