...
class BuildsService {
	...
	async getBuildInfoList(
	...
	): Promise<{ list: IBuildInfoItem[]; totalPages: number }> {
		let query =
		...
		const queryParams: any[] = [projectId, false];
		...
		if (filter.page !== -1) {
			...
			queryParams.push(String(filter.page * 10));
			queryParams.push(`10`);
		}
	...
	private getGithubConclusionFromReportStatus(reportStatus: BuildReportStatusEnum): GithubCheckConclusionEnum {
		switch (reportStatus) {
			...
			default:
				throw Error("Invalid value for report status");
		}
	}
