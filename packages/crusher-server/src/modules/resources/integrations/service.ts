...
class IntegrationsService {
	...
	async addIntegration(integrationConfig: SlackOAuthResponse, projectId: number) {
		return this.dbManager.insert(`INSERT INTO crusher.integrations (project_id, meta) VALUES (?, ?)`, [
			projectId,
	...
	async updateIntegration(integrationConfig: SlackOAuthResponse, id: number) {
		return this.dbManager.update(`UPDATE crusher.integrations SET meta = ? WHERE id = ?`, [JSON.stringify(integrationConfig), id]);
	}

	async deleteIntegration(id: number) {
		return this.dbManager.delete(`DELETE FROM crusher.integrations WHERE id = ?`, [id]);
	}

	@CamelizeResponse()
	async getSlackIntegration(projectId: number): Promise<KeysToCamelCase<IIntegrationsTable>> {
		return this.dbManager.fetchSingleRow("SELECT * FROM crusher.integrations WHERE project_id = ? AND integration_name = ?", [
			projectId,
	...
	async getListOfIntegrations(projectId: number): Promise<Array<KeysToCamelCase<IIntegrationsTable>>> {
		return this.dbManager.fetchAllRows("SELECT * FROM crusher.integrations WHERE project_id = ?", [projectId]);
	}
