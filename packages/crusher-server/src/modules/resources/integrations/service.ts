...
class IntegrationsService {
	...
	@CamelizeResponse()
	async getListOfIntegrations(projectId: number): Promise<KeysToCamelCase<IIntegrationsTable>[]> {
	...
	async saveSlackSettings(payload: { alertChannel: any; normalChannel: any }, projectId: number) {
		...
		integrationConfig.channel["alert"] = payload.alertChannel || null;
		integrationConfig.channel["normal"] = payload.normalChannel || null;

		return this.updateIntegration(integrationConfig, existingSlackIntegration.id);
	}

	async postSlackMessageIfNeeded(projectId: number, blocks: any[], type: "alert" | "normal") {
