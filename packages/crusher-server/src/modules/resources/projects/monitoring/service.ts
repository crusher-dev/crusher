...
class ProjectMonitoringService {
	...
	async getMonitoringList(projectId: number): Promise<Array<KeysToCamelCase<IMonitoringTable>>> {
		return this.dbManager.fetchAllRows("SELECT * FROM crusher.monitorings WHERE project_id = ?", [projectId]);
	}

	async createMonitoring(payload: ICreateMonitoringPayload): Promise<{ insertId: number }> {
		return this.dbManager.insert("INSERT INTO crusher.monitorings (project_id, environment_id, test_interval) VALUES (?, ?, ?)", [
			payload.projectId,
	...
	async getMonitoring(monitoringId: number): Promise<KeysToCamelCase<IMonitoringTable>> {
		return this.dbManager.fetchSingleRow("SELECT * FROM crusher.monitorings WHERE id = ?", [monitoringId]);
	}

	@CamelizeResponse()
	async getQueuedMonitoringDetails(): Promise<IQueuedMonitoringsDetails> {
		return this.dbManager.fetchAllRows(
			`SELECT monitorings.id id, e.user_id user_id, monitorings.project_id project_id, monitorings.environment_id environment_id, monitorings.test_interval test_interval, monitorings.last_cron_run last_cron_run, e.name environment_name, e.browser environment_browser, e.vars environment_vars FROM crusher.monitorings INNER JOIN environments e on monitorings.environment_id = e.id WHERE EXTRACT(EPOCH FROM (NOW() - monitorings.last_cron_run)) > monitorings.test_interval`,
		);
	}

	async updateLastCronMarker(monitoringId: number) {
		return this.dbManager.update(`UPDATE crusher.monitorings SET last_cron_run = NOW() WHERE id = ?`, [monitoringId]);
	}

	async deleteMonitoring(monitoringId: number) {
		return this.dbManager.delete("DELETE FROM crusher.monitorings WHERE id = ?", [monitoringId]);
	}

	async deleteAllMonitoringsOfEnvironment(environmentId: number) {
		return this.dbManager.delete("DELETE FROM crusher.monitorings WHERE environment_id = ?", [environmentId]);
	}
	...
	async updateMonitoring(payload: IUpdateMonitoringPayload, monitoringId: number) {
		...
		return this.dbManager.update(`UPDATE crusher.monitorings SET ${setQuery} WHERE id = ?`, [...setQueryValues, monitoringId]);
	}
}
