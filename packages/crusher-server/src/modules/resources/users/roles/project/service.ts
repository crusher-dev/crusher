...
class UserProjectRolesService {
	...
	async get(userId: number, projectId: number): Promise<KeysToCamelCase<IUserProjectRoleTable>> {
		return this.dbManager.fetchSingleRow(`SELECT * FROM public.user_project_roles WHERE user_id = ? AND project_id = ?`, [userId, projectId]);
	}

	async create(payload: ICreateUserProjectRole): Promise<{ insertId: number }> {
		return this.dbManager.insert(`INSERT INTO public.user_project_roles (user_id, project_id, role) VALUES (?, ?, ?)`, [
			payload.userId,
			payload.projectId,
