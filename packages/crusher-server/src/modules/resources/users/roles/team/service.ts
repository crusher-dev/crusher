...
class UserTeamRolesService {
	...
	async get(userId: number, teamId: number): Promise<KeysToCamelCase<IUserTeamRoleTable>> {
		return this.dbManager.fetchSingleRow(`SELECT * FROM crusher.user_team_roles WHERE user_id = ? AND team_id = ?`, [userId, teamId]);
	}

	async create(payload: ICreateUserTeamRole): Promise<{ insertId: number }> {
		return this.dbManager.insert(`INSERT INTO crusher.user_team_roles (user_id, team_id, role) VALUES (?, ?, ?)`, [
			payload.userId,
			payload.teamId,
