import { pxToRemValue } from "electron-app/src/lib/recorder/utils/helper";
...
const getItemsFromActionsData = (data: { [key: string]: { label: string; keywords: Array<string> } }) => {
...
const tranformActionsToList = (): Array<IActionItem> => {
    let finalItems = [];
    Object.keys(actionsData).map((category) => {
        ...
        if (isNotACategory) {
            ...
            finalItems.push(...Object.keys(actionsData[category]).map((actionId: any) => {
...
const transformResultsToActions = (results: Array<IActionItem>) => {
    const finalObj = {};
    results.map((item) => {
...
const filterActionsItems = (filter: string) => {
    ...
    const finalList = transformResultsToActions(searchItems.map((item) => { return listItems[item.refIndex]; }));
