...
class CloudCrusher {
	public static getTest: (testId: string) => Promise<Array<iAction>>  = createAuthorizedRequestFunc((authorizationOptions, testId) => {
		return axios.get<{ events: Array<iAction> }>(resolveToBackend(`/tests/${testId}`), {
	...
	public static createProject: (projectName: string) => Promise<any> = createAuthorizedRequestFunc((authorizationOptions, projectName) => {
		return axios.post(resolveToBackend("/projects/actions/create"), {
			name: projectName
		}, {
			...authorizationOptions
		}).then((res) => res.data);
	...
	public static runTests : (testIds: Array<string>, proxyConfig: any | null) => Promise<any> = createAuthorizedRequestFunc((authorizationOptions, testIds, proxyConfig) => {
		const store = getStore();
		const selectedProject = getCurrentSelectedProjct(store.getState() as any);
		if(!selectedProject) throw new Error("No project selected!");
		
		return axios.post(resolveToBackend(`/projects/${selectedProject}/tests/actions/run`), {
			testIds: Array.isArray(testIds) ? testIds.join(",") : testIds,
			proxyUrlsMap: getProxyFromTunnelData(proxyConfig)
		}, { ...authorizationOptions }).then((res) => res.data);
	});

	public static saveLocalBuildReport: (
		tests: Array<{ steps: Array<any>; testId: number; testName: string; status: "PASSED" | "FAILED" }>
	) => Promise<any> = createAuthorizedRequestFunc((authorizationOptions, tests) => {
		const store = getStore();
		const selectedProject = getCurrentSelectedProjct(store.getState() as any);
		if(!selectedProject) throw new Error("No project selected!");

		return axios.post(resolveToBackend(`/projects/${selectedProject}/builds/actions/create.local`), {
			tests: tests
		}, { ...authorizationOptions }).then((res) => res.data.insertId);
	});

	public static runDraftTest: (testId: string, proxyConfig: any | null) => Promise<any> = createAuthorizedRequestFunc((authorizationOptions, testId, proxyConfig) => {
		const store = getStore();
		const selectedProject = getCurrentSelectedProjct(store.getState() as any);
		if(!selectedProject) throw new Error("No project selected!");
		
		return axios.post(resolveToBackend(`/projects/${selectedProject}/tests/actions/runDraftTest`), {
			testId: testId,
			proxyUrlsMap: getProxyFromTunnelData(proxyConfig)
		}, {
			...authorizationOptions
	...
	public static getBuildReport: (buildId: string) => Promise<any> = createAuthorizedRequestFunc((authorizationOptions, buildId) => {
		return axios.get(resolveToBackend(`/builds/${buildId}/report`), {
			...authorizationOptions
		}).then((res) => res.data);
	});

	public static deleteTest: (testId: string) => Promise<any> = createAuthorizedRequestFunc((authorizationOptions, testId) => {
		return axios.post(resolveToBackend(`/tests/${testId}/actions/delete`), {}, {
			...authorizationOptions
		}).then((res) => res.data);
	});

	public static saveTest: (testPayload: { events: Array<iAction>; name: string; }, shouldAutorun: boolean) => Promise<any> = createAuthorizedRequestFunc(async (authorizationOptions, testPayload, shouldAutorun = true) => {
	...
	public static saveTestDirectly: (testPayload: { events: Array<iAction>; name: string; }, shouldAutorun: boolean, proxyConfig: any | null) => Promise<any> = createAuthorizedRequestFunc(async (authorizationOptions, testPayload, shouldAutorun = true, proxyConfig = null) => {
		const store = getStore();
		const selectedProject = getCurrentSelectedProjct(store.getState() as any);
		if(!selectedProject) throw new Error("No project selected!");
	...
	public static updateTest: (testId: string, testPayload: { events: Array<iAction>; }) => Promise<any> = createAuthorizedRequestFunc(async (authorizationOptions, testId, testPayload) => {
		const tempTest = await axios.post(resolveToBackend("/tests/actions/save.temp"), {
			events: testPayload.events,
		}, {
			...authorizationOptions
