...
const TemplatesModal = (props: iStartupModalProps) => {
	...
	return (
		<Modal modalStyle={modalStyle} onOutsideClick={props.handleClose}>
			...
			<div css={[css`height: 500rem; overflow-y: overlay;`, scrollBarStyle]}>
			<div css={formContainerStyle}>
				<hr css={css`margin-top: 8rem; border-color: rgb(255, 255, 255, 0.1); height: 0.1rem;`}/>
				<div css={css`margin-top: 16rem;`}>
					<div css={inputContainerStyle}>
						<div css={css`font-size: 13rem; color: rgb(255, 255, 255, 0.7); font-weight: 600;`}>Template name</div>
						<Input
							css={inputStyle}
					...
					<div css={[css`margin-top: 18rem;`]}>
						<div css={css`font-size: 13rem; color: rgb(255, 255, 255, 0.7); font-weight: 600;`}>Description</div>
						<textarea placeholder={"Enter description of template"} css={css`width: 100%; font-family: Cera Pro; height: 240rem; border-radius: 4rem; margin-top: 12rem; font-size: 14rem; color: #000; padding: 8rem;`}></textarea>
					</div>
				</div>


				<div onClick={setShowRequirements.bind(this, !showRequirements)} css={css`font-size: 15rem; font-weight: 600; color: #fff; margin-top: 30rem; font-family: Cera Pro; display: flex; :hover { opacity: 0.7 } `}>
                    <span>Requirements</span>
                    <div css={css`margin-left: auto; font-weight: 500;`}>
						<Conditional showIf={showRequirements}>
							<UpIcon css={css`margin-right: 4rem;`}/>
						</Conditional>
						<Conditional showIf={!showRequirements}>
							<DownIcon css={css`margin-right: 4rem;`}/>
						</Conditional>
                    </div>
                </div>
				<hr css={css`margin-top: 8rem; border-color: rgb(255, 255, 255, 0.1); height: 0.1rem;`}/>
