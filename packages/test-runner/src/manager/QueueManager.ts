...
const queue = new Queue('request-queue', {connection: REDDIS});
...
queue.client.then(async (reddisClient)=>{
 ...
 new Worker('request-queue', path.resolve('/code/src/services/codeRunnerWorker.ts'), { connection: reddisClient, concurrency: 3});
});
