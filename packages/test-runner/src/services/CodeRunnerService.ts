...
export default class CodeRunnerService {
	...
	async initDirectories(instanceId, platform) {
		...
		await shell.mkdir('-p', `/tmp/${instanceId}/${platform ? platform : JobPlatform.CHROME}/images`);
		await shell.mkdir('-p', `/tmp/${instanceId}/${platform ? platform : JobPlatform.CHROME}/video`);
	}

	async deleteDirectory(instanceId, platform) {
		await shell.rm('-rf', `/tmp/${instanceId}/${platform ? platform : JobPlatform.CHROME}/images`);
		await shell.rm('-rf', `/tmp/${instanceId}/${platform ? platform : JobPlatform.CHROME}/video`);
	}
}
