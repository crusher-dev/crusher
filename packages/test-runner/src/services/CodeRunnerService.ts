import {
  ...
  replaceWithPlaywrightWrapper,
} from "../../util/script";
import {
  uploadAllScreenshotsToTestBucket,
  uploadRecordedVideoToBucketIfAny,
} from "../../util/cloudBucket";
...
export default class CodeRunnerService {
  ...
  async runTestAndUploadOutputs(
    ...
    const finalCode = replaceWithPlaywrightWrapper(
      replaceImportWithRequire(code.replace(/\n/gm, ""))
    );
    try {
      const codeExecute = await new Function(
        ...
        __dirname,
        platform,
        this.logStep.bind(this, testType, testId)
      );

      const images = await uploadAllScreenshotsToTestBucket(
        destination,
        instanceId,
        platform as JobPlatform
      );
      const video = await uploadRecordedVideoToBucketIfAny(
        destination,
        instanceId,
        platform as JobPlatform
      );
      ...
      let video = await uploadRecordedVideoToBucketIfAny(
        destination,
        instanceId,
        platform as JobPlatform
      );
  ...
  async initDirectories(instanceId, platform) {
    ...
    await shell.mkdir(
      "-p",
      `/tmp/images/${instanceId}/${platform ? platform : JobPlatform.CHROME}`
    );
    await shell.mkdir(
      "-p",
      `/tmp/video/${instanceId}/${platform ? platform : JobPlatform.CHROME}`
    );
  }
