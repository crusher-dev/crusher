...
import {
    GITHUB_CHECK_STATUS, TEST_TYPES,
} from '../constants';
...
const videoProcessingQueue = new Queue('video-processing-queue', {connection: REDDIS as any});
...
module.exports = async (bullJob) => {
    ...
    try {
        const {images, logs, video} = await codeRunnerService.runTestAndUploadOutputs(test.testType, test.id, test.code, instanceId as number, `${test.testType ? test.testType : TestTypes.SAVED}/${test.id}/${instanceId}`, platform ? platform : JobPlatform.CHROME);
