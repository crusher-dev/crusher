...
const jsCode = codeGenerator.parse(events as any);

fs.writeFileSync(
	"out.js",
	beautify(
		"(async() => { function logStep(...args){console.log('LOG', args);}\n" +
			"function handleImageBuffer(...args){return 'hello';}\n" +
			beautify(jsCode).replace(/crusher\-runner\-utils\.ts\/src\/index\.ts/g, "../../output/crusher-runner-utils.ts/") +
			"})()",
	),
);
