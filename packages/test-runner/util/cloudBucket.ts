...
import {CLOUD_API_CREDENTIALS, CLOUD_PROJECT_ID, GCP_IMAGES_BUCKET} from "../../crusher-shared/config/google_cloud";
import {JobPlatform} from "../src/interfaces/JobPlatform";
import * as AWS from "aws-sdk";
import {AWS_BUCKET_REGION, AWS_S3_VIDEO_BUCKET} from "../../crusher-shared/config/aws_bucket";
...
export async function uploadFileToAwsBucket(s3Bucket: any, filePath: string, fileName: string, destination: string = "/") {
	...
	return new Promise((resolve, reject)=>{
		...
		console.log({Bucket: VIDEO_BUCKET_NAME, Key: destination + "/" + fileName, filePath});
		s3Bucket.upload({Bucket: VIDEO_BUCKET_NAME, Key: destination + "/" + fileName, Body: fileStream}, function(err: Error, data: any){
	...
	try {
		...
		for (let image of images) {
			...
			if(signedUrls && signedUrls.length) {
				publicImageUrls.push({name: image,url: signedUrls[0] + `&sbtnca=${Date.now()}`});
			}
		}
...
export async function uploadRecordedVideoToBucketIfAny(typeWithInstanceId: string, instanceId: number, platform: JobPlatform){
	try {
		...
		for (let video of videos) {
			const signedUrl = await uploadFileToAwsBucket(s3BucketService, `/tmp/video/${instanceId}/${platform}/${video}`, `${instanceId}.mp4.raw`, typeWithInstanceId);
			publicVideoUrls.push({name: video,url: signedUrl + `&sbtnca=${Date.now()}`});
		}
