...
export async function uploadFileToAwsBucket(
  s3Bucket,
  filePath: string,
  fileName: string,
  destination: string = "/"
...
export async function uploadAllScreenshotsToTestBucket(
  typeWithInstanceId: string,
  instanceId: number,
  ...
  try {
    ...
    for (let image of images) {
      await uploadFileToBucket(
        testImagesBucket,
        `/tmp/images/${instanceId}/${platform}/` + image,
        image,
        typeWithInstanceId
      );
...
export async function uploadRecordedVideoToBucketIfAny(
  typeWithInstanceId: string,
  instanceId: number,
  platform: JobPlatform
) {
